...
full_geography = select(cross_section,municipality,lat_mean,lon_mean,popn1993,ruggedness,slope,V_cum)
full_geography <- mutate(full_geography,ADM2_PCODE = purrr::map_chr(municipality,addCO))
...
cross_section_merged <- dplyr::select(cross_section_merged,c("ADM2_PCODE","admn2Nm","latnum","lonnum","has_road","municipality","ruggedness_index","slope_mean","alt_mean","elevation_difference","is_forested"))
...
# Read in shapefile. 
muni_pol <- st_read("municipalities_shapefile.shp",stringsAsFactors=FALSE)
...
munigraph <- spdep::graph_from_adjacency_matrix(mat2,mode ="undirected",weighted = TRUE)
...
for(i in 1:nrow(cross_section_merged)){
  for(j in 1:nrow(cross_section_merged)){
    if(are.connected(munigraph,i,j) & i != j){
...
for(pair in adjacent_pairs){
    ...
    max_elevation <- max(cross_section_merged$alt_mean)
    max_ruggedness <- max(cross_section_merged$ruggedness_index)
    ...
    elevation_difference <- max((cross_section_merged$alt_mean[i]-cross_section_merged$alt_mean[j]),0)
