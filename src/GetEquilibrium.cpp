...
{
  private:
    double D_e, V, delta, tau_ell, r, u_bar, a0, a1, p, q, H;
  public:
                 ...
                 double p, double q, double H, double omega_n)
{
  ...
  MigrationIntegrand g(D_e, V, delta, tau_ell, r, u_bar, a0, a1, p, q, H);
  double err_est;
  int err_code;
                        ...
                        double a1, double p, double q, double H, double omega_n)
{
  ...
  int i = 0;
  double x = 0.0;
  double f = get_Dstar(x, V, delta, tau_ell, tau_n, r, u_bar, a0, a1, p, q, H, omega_n);
  while((i < max_iter) & (std::abs(f - x) > tol)){
    x = f;
    f = get_Dstar(x, V, delta, tau_ell, tau_n, r, u_bar, a0, a1, p, q, H, omega_n);
    i++;
  }
  return(x);
}
...
{
  double Dstar = get_migration_eq(V, delta, tau_ell, tau_n, r, u_bar, a0,
                        a1, p, q, H, omega_n);
  ExpropriationIntegrand g(Dstar, V, delta, tau_ell, r, u_bar, a0, a1, p, q, H);
  double err_est;
  int err_code;
